...
class VAE(nn.Module):
    def __init__(self, input_dim: int = 1, latent_dim: int = 128, hidden_dim: List[int] = None) -> None:
        ...
        self.hidden_dim = hidden_dim if hidden_dim is not None else [32, 64, 128, 256, 512, 1024]
        ...
        self.mu = nn.Linear(self.hidden_dim[-1] * 4, latent_dim)  # Latent space dimension
        self.var = nn.Linear(self.hidden_dim[-1] * 4, latent_dim)

        # Decoder definition (reversed order of hidden dimensions)
        self.linear_decoder = nn.Linear(latent_dim, self.hidden_dim[-1] * 4)

        decoder_modules = []
        hidden_decoder = self.hidden_dim[::-1]
        ...
        self.final_layer = nn.Sequential(
            nn.ConvTranspose2d(hidden_decoder[-1], hidden_decoder[-1], kernel_size=3, stride=2, padding=1,
                               output_padding=1),
            nn.BatchNorm2d(hidden_decoder[-1]),
            nn.LeakyReLU(),
            nn.Conv2d(hidden_decoder[-1], out_channels=1, kernel_size=3, padding=1),
            nn.Tanh())

    def encode(self, _input: torch.Tensor) -> list[Linear]:
        """
        ...
        _encode = self.encoder(_input)
        print(_encode.shape)
        result = torch.flatten(_encode, start_dim=1)
        print(result.shape)
        mu = self.mu(result)
        var = self.var(result)
    ...
    def reparameterize(self, mu: Linear, var: Linear) -> torch.Tensor:
        ...
        epsilon = torch.randn(mu.size()).to(self.device)  # Sample random noise
        std = torch.exp(0.5 * var).to(self.device) # Standard deviation from variance
        z = mu + epsilon * std
        return z
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        """
        result = self.linear_decoder(z)
        result = result.view(-1, 1024, 2, 2)
        result = self.decoder(result)
        return self.final_layer(result)

    def forward(self, _input: torch.Tensor) -> tuple[Any, Any, Any]:
        mu, var = self.encode(_input)
